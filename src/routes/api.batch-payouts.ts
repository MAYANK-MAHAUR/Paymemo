...
const batchRecordSchema = z.object({
  id: z.string().min(1),
  walletAddress: z.string().min(1),
  type: z.literal("batch-payout").default("batch-payout"),
  publicData: z.record(z.unknown()),
  encryptedMetadata: encryptedMetadataSchema,
  status: z.enum(["draft", "intent", "dispatching", "confirmed", "failed"]).default("draft"),
  createdAt: z.string().min(1),
  updatedAt: z.string().min(1),
});

export const Route = createFileRoute("/api/batch-payouts")({
  server: {
    handlers: {
      GET: async ({ request }: { request: Request }) => {
        ...
        const records = await listEncryptedDomainRecords(walletAddress, "batch-payout");
      ...
      POST: async ({ request }: { request: Request }) => {
        ...
        if (!parsed.success) {
          return Response.json(
            { error: "Invalid encrypted batch payout record", issues: parsed.error.flatten() },
            { status: 400 },
          );
        }
        ...
        try {
          ...
          const message = error instanceof Error ? error.message : "Unable to save batch payout";
